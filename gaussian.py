...
def prior_mean(x, m=0.0, lam=1, beta=1):
    return scipy.stats.norm(x, loc=m, scale=1/(beta*lam))
...
def posterior_mean(x, lam=1, data=None, m=None, lam=None, beta=None):
    N = len(data)
    mu_hat = (np.sum(data ** 2) + beta * m) / (beta + N)
    beta_hat = beta + N
    return scipy.stats.norm(x, loc=mu_hat, scale=1/(beta_hat*lam))
...
def posterior_precision(x, data=None, a=None, b=None):
    ...
    return scipy.stats.gamma.pdf(x, a_hat, loc=0, scale=b_hat)


def likelihood(x, mu=0.0, lam=1.0):
    return scipy.norm.pdf(x, loc=mu, scale=1/lam)
...
def pred_dist(x, m=None, beta=None, a=None, b=None):
    ...
    return scipy.stats.t.pdf(x, df, loc, scale)
